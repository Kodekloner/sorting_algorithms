 ...
 */
int partition(int *array, size_t sz, int x, int y)
{
	int *mid, up, down;
 ...
 */
void sort(int *array, size_t sz, int x, int y)
{
	int part;

	if (y - x > 0)
	{
		part = partition(array, sz, x, y);
		sort(array, sz, x, part - 1);
		sort(array, sz, part + 1, y);
	}
}
